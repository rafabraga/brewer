...
public class UsuariosImpl implements UsuariosQueries {
    ...
    @Override
    public List<String> permissoes(final Usuario usuario) {
        return this.manager
    ...
    @Override
    public Page<Usuario> filtrar(final UsuarioFilter filtro, final Pageable pageable) {
        ...
        final List<Usuario> filtrados = criteria.list();
    ...
    private void adicionarFiltro(final UsuarioFilter filtro, final Criteria criteria) {
        if (filtro != null) {
            ...
            if ((filtro.getGrupos() != null) && !filtro.getGrupos().isEmpty()) {
                final List<Criterion> subqueries = new ArrayList<>();
                for (final Long codigoGrupo : filtro.getGrupos().stream().mapToLong(Grupo::getCodigo).toArray()) {
                    final DetachedCriteria dc = DetachedCriteria.forClass(UsuarioGrupo.class);
                ...
                final Criterion[] criterions = new Criterion[subqueries.size()];
    ...
    @SuppressWarnings("deprecation")
    private Long total(final UsuarioFilter filtro) {
        final Criteria criteria = this.manager.unwrap(Session.class).createCriteria(Usuario.class);
        this.adicionarFiltro(filtro, criteria);
